...
public class Warrior { //정의부, 실행은 Main에서 할 예정임
	...
	//public : 모든 영역에서 수정/조회 가능, private : 자기자기신만 수정/ 조회가능
	protected int hp;
	protected  int stranght;
	protected  String name;
	protected  int lv;
	protected  int money;
	
	// 생성자를 만들어주시되, name만 사용자에게 입력받고
	// 나머지는 hp20, str 3, lv 1, money 0
	...
	// 이 메서드는 실행시 사냥하는 상황을 구현하며
	// 체력이 2 깍이고 돈을 10씩 획득합니다
	// 사냥결과 체력이 X가 되었고, 소지금은 Y입니다 라고 출력합니다.
	
	// 생성자를 비롯한 메서드는 "행동"을 나타내고
	// 이런 행동의 성공/실패여부는 오롯이 정보(변수)에 의해 결정되며 
	// 시도자체에 대해서는 제한을 두지 않습니다.

	public void hunt() {
	hp -= 2;
	money += 10;
	System.out.println("사냥결과 체력이 " + hp + " 가 되었고, 소지금이 " + money + "입니다.");
